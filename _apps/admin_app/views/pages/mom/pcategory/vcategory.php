<section class="content-header">
	<h1>Kategori Alumni <small>Memorial SMPN 1 &amp; SMAN 1 Samarinda</small><button class="btn btn-sm pull-right bg-pusam" data-toggle="modal" data-target="#modal-add-category">+ Tambah</button></h1>
</section>
<section class="content" style="min-height: 50px;">
	<form id="form-filter" action="#" class="form-horizontal">					
		<div class="col-md-3">
			<div class="form-group">
                <select class="form-control select2" id="alumni_search" >
                    <option></option>
                    <?php
                    	foreach ($alumni_list as $row) {
                    		echo '<option value="'.$row->id_alumni.'">'.$row->name.'</option>';
                    	}
                    ?>
                </select>
			</div>
...
<section class="content">	
	<div class="box no-radius">
		<div class="box-body">
			<table id="table_category" class="table table-hover table-bordered" width="100%">
   ...
   <div class="modal-dialog center" role="document">
      	<div class="modal-content">
         	<div class="modal-header no-border">
	            ...
	            <h4 class="modal-title">Tambah Kategori Alumni</h4>
         	</div>
         	<form id="form-add-category" method="post" action="#">
	            <div class="modal-body">
	            	<div class="form-group">
						<label class="control-label">Alumni</label>
						<select class="form-control select2 required" name="id_alumni">
							<option></option>
							<?php
								foreach ($alumni_list as $row) {
									echo '<option value="'.$row->id_alumni.'">'.$row->name.'</option>';
	            ...
	            <div class="modal-footer no-border">
	               ...
	               <button type="button" id="btn_add_category" class="btn btn-danger">Simpan</button>
	            </div>
	        </form>
      	</div>
   	</div>
   ...
   <div class="modal-dialog center" role="document">
      	<div class="modal-content">
         	<div class="modal-header no-border">
	            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
	            <span aria-hidden="true">&times;</span></button>
	            <h4 class="modal-title">Ubah Kategori Alumni</h4>
         	</div>
         	<form id="form-edit-category" method="post" action="#">
	            ...
	            <div class="modal-body">
	            	<div class="form-group">
						<label class="control-label">Alumni</label>
						<select class="form-control select2 required" name="id_alumni" id="id_alumni">
							<option></option>
							<?php
								foreach ($alumni_list as $row) {
									echo '<option value="'.$row->id_alumni.'">'.$row->name.'</option>';
	            ...
	            <div class="modal-footer no-border">
	               <button type="button" class="btn btn-default" data-dismiss="modal">Batal</button>
	               <button type="button" id="btn_edit_category" class="btn btn-danger">Simpan</button>
	            </div>
	        </form>
      	</div>
   	</div>
...
<script type="text/javascript">
	$(document).ready(function (){
		...
		$('#mom-alumni-category').addClass('active');
		...
		var table = $('#table_category').DataTable({
			...
			"ajax": {
				"url" : '<?=site_url()?>../../table/alumni/category',
				"type" : 'POST',
				data : function(data){ data.alumni_search = $("#alumni_search").val();data.name_search = $("#name_search").val();data.status_search = $("#status_search").val();},
		...
		$('#modal-edit-category').on('show.bs.modal', function (event) {
      	...
      	$('#btn_add_category').click(function(){
			...
			if($("#form-add-category").valid() == false){
				...
				$.post("<?=site_url();?>../../save/add/alumni/category",
				formData,
				function(data) {
					if(data == "Success"){
	    ...
	    $('#btn_edit_category').click(function(){
			...
			if($("#form-edit-category").valid() == false){
				...
				$.post("<?=site_url();?>../../save/edit/alumni/category",
				formData,
				function(data) {
					if(data == "Success"){
	...
	function removeData(id, name){
	    swal({
	        title: "",html: '<i class="fas fa-question-circle f40 margin10 text-red"></i><br>Hapus kategori alumni ini (<b>'+name+'</b>)?',type: "",
			...
			if (result.value){
			    $.ajax({
					url: "<?=site_url()?>../../save/delete/alumni/category",
					type: "post",
