<?php defined('BASEPATH') OR exit('No direct script access allowed');
    ...
    class Cdetail extends CI_Controller{
        ...
        function __construct(){
            ...
            $this->load->model(['msm/mod_detail']);
        ...
        public function detail_list(){
            $data = array(
                ...
                'content'  => 'pages/msm/pdetail/vdetail',
                'category_list' => $this->mod_global->category_list(),
                'css_script' => array(
        ...
        public function table_detail(){
            ...
            $detail_list = $this->mod_detail->get_detail_list($length, $start);

            foreach ($detail_list as $field){
                ...
                if ($field->type == 'video') {
                    $item = '
                        <a href="#" data-tooltip="Lihat" data-toggle="modal" data-target="#modal-vid" data-vid="../../_assets/img/museum/'.$field->type.'/'.$field->file.'">
                            <img src="../../_assets/img/museum/thumb/'.$field->thumbnail.'" width="60" >
                        </a>
                    ';
                } elseif ($field->type == 'gambar') {
                    $item = '
                        <a href="#" data-tooltip="Lihat" data-toggle="modal" data-target="#modal-img" data-img="../../_assets/img/museum/'.$field->type.'/'.$field->file.'">
                            <img src="../../_assets/img/museum/thumb/'.$field->thumbnail.'" width="60">
                        </a>';
                } else {
                    $item = '
                        <a href="#" data-tooltip="Lihat" data-toggle="modal" data-target="#modal-doc" data-doc="../../_assets/img/museum/'.$field->type.'/'.$field->file.'">
                            <img src="../../_assets/img/museum/thumb/'.$field->thumbnail.'" width="60" >
                        </a>
                    ';
                }
                if ($this->checkAccess->id_level != 1) {
                    ...
                    $btnGod = '<a href="#" class="btn btn-xs bg-red" data-tooltip="Hapus" onclick="removeData(\''.$this->my_encryption->encrypt_decrypt('encrypt',$field->id_detail).'\',\''.$field->detail_name.'\')"><i class="fas fa-times"></i></a>';
                }
                $start++;
                $row             = array();
                $row['no']       = $start;
                $row['category'] = $field->category_name;
                $row['name']     = $field->detail_name;
                $row['desc']     = $field->description;
                $row['date']     = $field->tanggal;
                $row['type']     = $field->type;
                $row['thumb']    = '<a href="#" data-tooltip="Lihat" data-toggle="modal" data-target="#modal-img" data-img="../../_assets/img/museum/thumb/'.$field->thumbnail.'"><img src="../../_assets/img/museum/thumb/'.$field->thumbnail.'" width="50"></a>';
                $row['item']   = $item;
                $row['status'] = '<span class="hidden">'.$field->status_active.'</span>'.$status;
                $row['action'] = '
                    <a href="#" class="btn btn-xs bg-blue" data-tooltip="Ubah" data-toggle="modal" data-target="#modal-edit-detail" data-id_detail="'.$this->my_encryption->encrypt_decrypt('encrypt',$field->id_detail).'" data-id_category="'.$field->id_category.'" data-name="'.$field->detail_name.'" data-desc="'.$field->description.'" data-date="'.$field->tanggal.'" data-type="'.$field->type.'" data-active="'.$field->status_active.'" data-file="../../_assets/img/museum/'.$field->type.'/'.$field->file.'" data-thumb="../../_assets/img/museum/thumb/'.$field->thumbnail.'"><i class="fas fa-pen f10"></i></a>
                    '.$btnGod.'
                    ';
                $data[]        = $row;
            };
            $output = array(
                ...
                "recordsTotal"    => $this->mod_detail->count_all_detail_list(),
                "recordsFiltered" => $this->mod_detail->count_filtered_detail_list(),
                "data"            => $data
            );
        ...
        public function save_add_detail(){
            ...
            for($i=0;$i<$count;$i++){
                ...
                $config['upload_path'] = '../../_assets/img/museum/'.$type.'/'; 
                ...
                if($this->upload->do_upload('file')){
                    ...
                    $src = '../../_assets/img/museum/'.$type.'/'.$uploadData['file_name'];
                    $thumb_target = '../../_assets/img/museum/thumb/'.$uploadData['file_name'];
                } else {
                ...
                if ($type == "gambar") {
                    ...
                    $aaa = array(
                        ...
                        'type'        => $type,
                        'file'        => $type.'-'.date('ymdHis').$extention1,
                        'thumbnail'   => $type.'-'.date('ymdHis').$extention1
                    );
                }
                if ($type != "gambar" && ($_FILES['addfile']['type'][1] == "image/jpeg" || $_FILES['addfile']['type'][1] == "image/png" ) ) {
                    ...
                    $aaa = array(
                        ...
                        'type'        => $type,
                        'file'        => $type.'-'.date('ymdHis').$extention1,
                        'thumbnail'   => $type.'-'.date('ymdHis').$extention2
                    );
                }
            }
            if ($count == 2) {
            ...
            $save = $this->mod_global->insert_all('msm_detail', $aaa);
            if ($save == true) {
                $dataLog = array(
                    'id_user'    => $this->session->userdata('id_user'),
                    'logs'       => 'Tambah Detail: '.$this->pregReps($this->input->post('name')),
        ...
        public function save_edit_detail(){
            $id_detail = $this->my_encryption->encrypt_decrypt('decrypt',$this->pregReps($this->input->post('id_detail')));
            $type = $this->pregReps($this->input->post('type'));
            $data = array();
            $src  = "";
            $thumb_target = "";
            ...
            if ($_FILES['editfile']['name'][0] == "" || $_FILES['editfile']['name'][1] == "") {
                ...
                $save = $this->mod_global->edit_all('id_detail', $id_detail, 'msm_detail', $alldata);
                if ($save == true) {
                    $dataLog = array(
                        'id_user'    => $this->session->userdata('id_user'),
                        'logs'       => 'Ubah Detail: '.$this->pregReps($this->input->post('name')),
                        'ip_address' => $this->input->ip_address()
                    );
                    $saveLog = $this->mod_global->insert_all('master_user_log', $dataLog);
                ...
                $getimage = $this->mod_detail->get_image($id_detail);
                if(file_exists('../../_assets/img/museum/'.$getimage->type.'/'.$getimage->file))
                    unlink('../../_assets/img/museum/'.$getimage->type.'/'.$getimage->file);
                if(file_exists('../../_assets/img/museum/thumb/'.$getimage->thumbnail))
                    unlink('../../_assets/img/museum/thumb/'.$getimage->thumbnail);
                $save = $this->mod_global->edit_all('id_detail', $id_detail, 'msm_detail', $alldata);
                if ($save == true) {
                    $dataLog = array(
                        'id_user'    => $this->session->userdata('id_user'),
        ...
        public function delete_detail(){
            ...
            $id_detail = $this->my_encryption->encrypt_decrypt('decrypt', $this->pregReps($this->input->post('id')));
            $name = $this->pregReps($this->input->post('name'));
            $data = array('isDelete' => 1, 'status_active' => 0);
            $isDelete = $this->mod_global->edit_all('id_detail', $id_detail, 'msm_detail', $data);
            if ($isDelete == true){
                $getimage = $this->mod_detail->get_image($id_detail);
                if(file_exists('../../_assets/img/museum/'.$getimage->type.'/'.$getimage->file))
                    unlink('../../_assets/img/museum/'.$getimage->type.'/'.$getimage->file);
