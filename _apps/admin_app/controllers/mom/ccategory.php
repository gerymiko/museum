<?php defined('BASEPATH') OR exit('No direct script access allowed');
    ...
    class Ccategory extends CI_Controller{
        ...
        function __construct(){
            ...
            $this->load->model(['mom/mod_category']);
        ...
        public function category_alumni_list(){
            $data = array(
                'header'   => 'pages/ext/header',
                'footer'   => 'pages/ext/footer',
                'topmenu'  => 'pages/ptopbar/vtopbar',
                'sidemenu' => 'pages/psidebar/vsidebar',
                'content'  => 'pages/mom/pcategory/vcategory',
                'alumni_list' => $this->mod_global->mom_alumni_list(),
                'css_script' => array(
        ...
        public function table_category_alumni(){
            $data   = array();
            $start  = $this->pregRepn($this->input->post('start'));
            $length = $this->pregRepn($this->input->post('length'));
            if ($start == null || $length == null || $start == "" || $length == ""){ show_404();exit(); }
            $category_alumni = $this->mod_category->get_category_alumni($length, $start);

            foreach ($category_alumni as $field){
                ...
                $row           = array();
                $row['no']     = $start;
                $row['group']  = $field->alumni_name;
                $row['name']   = $field->category_name;
                $row['desc']   = $field->description;
                $row['status'] = '<span class="hidden">'.$field->status_active.'</span>'.$status;
                $row['action'] = '
                    <a href="#" class="btn btn-xs bg-blue" data-tooltip="Ubah" data-toggle="modal" data-target="#modal-edit-category" data-id_category="'.$this->my_encryption->encrypt_decrypt('encrypt',$field->id_category).'" data-id_alumni="'.$field->id_alumni.'" data-name="'.$field->category_name.'" data-desc="'.$field->description.'" data-active="'.$field->status_active.'"><i class="fas fa-pen f10"></i></a>
                    '.$btnGod.'
                    ';
                $data[]        = $row;
            };
            $output = array(
                ...
                "recordsTotal"    => $this->mod_category->count_all_category_alumni(),
                "recordsFiltered" => $this->mod_category->count_filtered_category_alumni(),
                "data"            => $data
            );
            echo json_encode($output);
        }

        public function save_add_category_alumni(){
            $data = array(
                'id_alumni'   => $this->pregRepn($this->input->post('id_alumni')),
                'name'        => $this->pregReps($this->input->post('name')),
                'description' => $this->pregReps($this->input->post('description'))
            );
            $checkdata = $this->mod_category->check_data($this->pregReps($this->input->post('name')));
            if ($checkdata != false) {
                echo 'register';exit();
            }
            $edit = $this->mod_global->insert_all('mom_alumni_category', $data);
            if ($edit == true) {
                $dataLog = array(
                    'id_user'    => $this->session->userdata('id_user'),
                    'logs'       => 'Tambah Kategori Alumni : '.$this->pregReps($this->input->post('name')),
        ...
        public function save_edit_category_alumni(){
            $id_category = $this->my_encryption->encrypt_decrypt('decrypt', $this->pregReps($this->input->post('id_category')));
            $data = array(
                'id_alumni'     => $this->pregRepn($this->input->post('id_alumni')),
                'name'          => $this->pregReps($this->input->post('name')),
                'description'   => $this->pregReps($this->input->post('description')),
            ...
            $edit = $this->mod_global->edit_all('id_category', $id_category, 'mom_alumni_category', $data);
            if ($edit == true) {
                $dataLog = array(
                    'id_user'    => $this->session->userdata('id_user'),
                    'logs'       => 'Ubah Kategori Alumni : '.$this->pregReps($this->input->post('name')),
                    'ip_address' => $this->input->ip_address()
                );
                $saveLog = $this->mod_global->insert_all('master_user_log', $dataLog);
                if ($saveLog == true) {
        ...
        public function delete_category_alumni(){
            ...
            $isDelete = $this->mod_global->edit_all('id_category', $id_category, 'mom_alumni_category', $data);
            if ($isDelete == true){
                $dataLog = array(
                    'id_user'    => $this->session->userdata('id_user'),
                    'logs'       => 'Hapus Kategori Alumni : '.$name,
                    'ip_address' => $this->input->ip_address()
                );
                $saveLog = $this->mod_global->insert_all('master_user_log', $dataLog);
                if ($saveLog == true) {
