<?php defined('BASEPATH') OR exit('No direct script access allowed');
    ...
    class Cdetail extends CI_Controller{
        ...
        function __construct(){
            ...
            $this->load->model(['mom/mod_detail']);
        ...
        public function detail_alumni_list(){
            $data = array(
                'header'   => 'pages/ext/header',
                'footer'   => 'pages/ext/footer',
                'topmenu'  => 'pages/ptopbar/vtopbar',
                'sidemenu' => 'pages/psidebar/vsidebar',
                'content'  => 'pages/mom/pdetail/vdetail',
                'category_list' => $this->mod_global->mom_category(),
                'css_script' => array(
        ...
        public function table_detail_alumni(){
            $data   = array();
            $start  = $this->pregRepn($this->input->post('start'));
            $length = $this->pregRepn($this->input->post('length'));
            if ($start == null || $length == null || $start == "" || $length == ""){ show_404();exit(); }
            $detail_alumni = $this->mod_detail->get_detail_alumni($length, $start);

            foreach ($detail_alumni as $field){
                ...
                $row            = array();
                $row['no']      = $start;
                $row['category'] = $field->alumni_name.' ( '.$field->category_name.' )';
            ...
            $output = array(
                ...
                "recordsTotal"    => $this->mod_detail->count_all_detail_alumni(),
                "recordsFiltered" => $this->mod_detail->count_filtered_detail_alumni(),
                "data"            => $data
            );
        ...
        public function save_add_detail_alumni(){
            ...
            $save = $this->mod_global->insert_all('mom_alumni_detail', $alldata);
            if ($save == true) {
                $dataLog = array(
        ...
        public function save_edit_detail_alumni(){
            ...
            if ($_FILES['editfile']['name'][0] == "" || $_FILES['editfile']['name'][1] == "") {
                ...
                $save = $this->mod_global->edit_all('id_detail', $id_detail, 'mom_alumni_detail', $alldata);
                if ($save == true) {
                    $dataLog = array(
                        'id_user'    => $this->session->userdata('id_user'),
        ...
        public function delete_detail_alumni(){
            ...
            $isDelete = $this->mod_global->edit_all('id_detail', $id_detail, 'mom_alumni_detail', $data);
            if ($isDelete == true){
                $getimage = $this->mod_detail->get_image($id_detail);
                if(file_exists('../../_assets/img/memorial/'.$getimage->type.'/'.$getimage->file))
                    unlink('../../_assets/img/memorial/'.$getimage->type.'/'.$getimage->file);
