<?php defined('BASEPATH') OR exit('No direct script access allowed');
	...
	class Mod_ensiklopedia extends CI_Model {
	    ...
	    private function _get_ensiklopedia_list(){
	    	...
	    	$this->db->select('a.id_menu, a.name as submenu_name, b.id_ensiklo, b.name as ensiklo_name, b.description, b.tanggal, b.type, b.thumbnail, b.file, b.status_active');
	    	$this->db->from('mom_submenu a');
	        $this->db->join('mom_ensiklopedia b', 'a.id_menu = b.id_menu AND a.status_active = 1 AND b.status_active = 1', 'inner');
	        $this->db->where($datax);
	        $i = 0;
	    ...
	    function get_ensiklopedia_list($length, $start){
	        $this->_get_ensiklopedia_list();
	    ...
	    function count_filtered_ensiklopedia_list(){
	        $this->_get_ensiklopedia_list();
	        $query = $this->db->get();
	        return $query->num_rows();
	    }

	    function count_all_ensiklopedia_list(){
	    	$this->_get_ensiklopedia_list();
	    	return $this->db->count_all_results();
	    }

	    function get_image($id_ensiklo){
	    	$datax = array('id_ensiklo' => $this->pregRepn($id_ensiklo));
	    	$query = $this->db->select('type, thumbnail, file')
	    		->from('mom_ensiklopedia')
	    		->where($datax)
	    		->get();
	    	if($query->num_rows() > 0 ){
	            return $query->row(); 
	    ...
	    // 	$query = $this->db->from('mom_ensiklopedia')
	    // 		->where($datax)
	    // 		->get();
	    // 	if($query->num_rows() > 0 ){
	    //         return $query->row();
