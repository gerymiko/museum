<?php defined('BASEPATH') OR exit('No direct script access allowed');
	...
	class Mod_category extends CI_Model {
		...
		var $order      = array('a.id_alumni' => 'ASC');
	    ...
	    private function _get_category_alumni(){
	    	...
	    	if($this->pregRepn($this->input->post('alumni_search')))
				$this->db->where('a.id_alumni', $this->pregRepn($this->input->post('alumni_search')));
	    	$datax = array('a.isDelete' => 0, 'b.isDelete' => 0);
	    	$this->db->select('a.id_alumni, a.name as alumni_name, b.id_category, b.name as category_name, b.description, b.status_active');
	        $this->db->from('mom_alumni a');
	        $this->db->join('mom_alumni_category b', 'a.id_alumni = b.id_alumni AND a.status_active = 1 AND b.status_active = 1');
	    ...
	    function get_category_alumni($length, $start){
	        $this->_get_category_alumni();
	    ...
	    function count_filtered_category_alumni(){
	        $this->_get_category_alumni();
	        $query = $this->db->get();
	        return $query->num_rows();
	    }

	    function count_all_category_alumni(){
	    	$this->_get_category_alumni();
	    ...
	    function check_data($name){
	    	...
	    	$query = $this->db->from('mom_alumni_category')
	    		->where($datax)
	    		->get();
	    	if($query->num_rows() > 0 ){
	            return $query->row();
