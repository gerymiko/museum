<div class="slide">
   <div class="content transition-fade" id="swup"><br>
      <h4 class="detail">Galeri <?=$detail->category_name?></h4>
      <div class="fourth-content">
         <div class="container-fluid">
            <div class="row">
               <div class="col-sm-6">
                  <div class="main-btn transition-fade"><a href="<?=site_url('../../category/').$this->my_encryption->encrypt_decrypt('encrypt', $detail->id_alumni)?>">Kembali</a></div>
            ...
            <?php foreach ($detail_list as $row) { if ($row->type == 'video') { ?>
               <div style="display:none;" id="<?=$row->type.$row->id_detail?>">
                  <video class="lg-video-object lg-html5" controls preload="none">
                     <source src="<?=site_url('dshow/file/').$this->my_encryption->encrypt_decrypt('encrypt',$row->id_detail)?>" type="video/mp4">
               ...
               <ul id="html5-videos" class="list-unstyled row">
                  <?php 
                     foreach ($detail_list as $row) { 
                        ...
                        if ($row->type == 'video') {
                           echo '
                           <li class="col-xs-6 col-sm-4 col-md-3 video red" data-color="red" data-poster="'.site_url('dshow/thumb/').$this->my_encryption->encrypt_decrypt('encrypt',$row->id_detail).'" data-sub-html="'.$row->type.' '.$row->description.'" data-html="#'.$row->type.$row->id_detail.'" >
                              <a href="#">
                                 <div class="caption hidden">
                                    <h5>'.$row->detail_name.'</h5><p>'.$row->description.'</p>
                                 </div>
                                 <img class="img-responsive" src="'.site_url('dshow/thumb/').$this->my_encryption->encrypt_decrypt('encrypt',$row->id_detail).'">
                           ...
                           echo '
                           <li class="col-xs-6 col-sm-4 col-md-3 video green" data-color="green" data-src="'.site_url('dshow/file/').$this->my_encryption->encrypt_decrypt('encrypt',$row->id_detail).'">
                              <a href="#">
                                 <div class="caption hidden">
                                    <h5>'.$row->detail_name.'</h5><p>'.$row->description.'</p>
                                 </div>
                           ...
                           echo '
                           <li class="col-xs-6 col-sm-4 col-md-3 video blue" data-color="blue" data-iframe="true" data-src="../../../../_assets/img/memorial/'.$row->type.'/'.$row->file.'">
                              <a href="#">
                                 <div class="caption hidden">
                                    <h5>'.$row->detail_name.'</h5><p>'.$row->description.'</p>
                                 </div>
